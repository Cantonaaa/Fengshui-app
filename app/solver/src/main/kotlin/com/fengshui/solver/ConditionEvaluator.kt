...
object ConditionEvaluator {
    ...
    /** 评估一条规则条件是否违规。 */
    fun violated(cond: RuleCondition, facts: RoomFacts, context: Map<String, String> = emptyMap()): Boolean {
        // require：全部存在
        ...
        for ((objType, facts_) in cond.spatial) {
            for (o in facts.objects.filter { it.type == objType }) {
                if (facts_.any { evalFact(it, o, facts) }) return true
            }
        }
        return false
    }
    ...
    fun evalFact(fact: String, o: Furniture, facts: RoomFacts): Boolean {
        return when (fact) {
    ...
    private val badSectors = setOf("南", "东南", "东")   // 简化：凶位示例
    private val goodSectors = setOf("北", "西北", "西", "西南", "东北")
}
