...
| 2 | **A1.3 户型多边形**：地板平面+墙平面交线→房间边界多边形；点云后处理墙平面裁剪（去玻璃透反射鬼影点）| 30秒扫描出可辨认房型；**含玻璃窗房间单列验收**（窗洞/墙线不歪、杂点被裁）|
| 3 | **K3 视觉模型（一步到位）**：YOLO-World 单模型 + **固定小词表**（仅规则类 11+ 类）；**重参化**（词汇嵌入固化）→ ONNX → **NCNN**（端上运行时，TFLite 官方支持未完成）；扫描用**选帧检测**（2-8FPS 即可，房间形状不受影响）| 端上可检测规则类家具；真机帧率实测；**不达标则 GroundingDINO 自动标注→蒸馏专类小模型** |
| 4 | **A1.4 物体3D定位+尺寸**：检测框中心+相机位姿+点云/平面反投影→3D坐标；多帧三角化+地板约束→实测尺寸(高置信)；低置信回退类型默认；窗户以视觉检测为主 | 床/门/灶等可标位；家具尺寸可估（占位/贴墙规划够用）|
